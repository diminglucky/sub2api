...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	upstreamBalanceService := service.NewUpstreamBalanceService(accountRepository, proxyRepository, balanceNotifyService, leaderLockCache)
	...
	imageStudioGalleryStore := repository.NewImageStudioGalleryStore(redisClient)
	imageStudioGalleryService := service.NewImageStudioGalleryService(imageStudioGalleryStore, imageStorageSettingService)
	imageStudioGalleryHandler := handler.NewImageStudioGalleryHandler(imageStudioGalleryService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, lotteryHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, imageStudioGalleryHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
