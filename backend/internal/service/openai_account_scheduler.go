...
func accountSupportsOpenAICapabilities(account *Account, requestedModel string, requiredCapability OpenAIEndpointCapability, requiredImageCapability OpenAIImagesCapability) bool {
	...
	if requiredImageCapability != "" && !accountSupportsOpenAIImagesModel(account, requestedModel) {
		return false
	}
...
func accountSupportsOpenAIImagesModel(account *Account, requestedModel string) bool {
	if account == nil {
		return false
	}
	...
	// OAuth image requests are transformed into OpenAI Responses image_generation
	// tool calls, where third-party/custom image model names are not safe. APIKey
	// accounts hit the configured Images-compatible upstream directly.
