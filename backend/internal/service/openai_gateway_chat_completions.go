...
func (s *OpenAIGatewayService) recordOpenAIStreamUpstreamError(c *gin.Context, account *Account, stream bool, upstreamRequestID, kind string, body []byte, message string) string {
	if c == nil {
...
func (s *OpenAIGatewayService) handleChatBufferedStreamingResponse(
	...
	if strings.TrimSpace(finalResponse.Status) == "failed" {
		...
		if openAICompatChatFailedEventShouldFailover(payload, message) {
...
func (s *OpenAIGatewayService) handleChatStreamingResponse(
	...
	processDataLine := func(payload string) bool {
		...
		if strings.TrimSpace(event.Type) == "response.failed" {
			...
			if openAICompatChatFailedEventShouldFailover(payloadBytes, message) {
