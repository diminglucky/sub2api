    ...
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input v-model.number="planForm.purchase_limit_per_user" type="number" min="0" class="input" />
...
const planForm = reactive({ name: '', group_id: null as number | null, description: '', price: 0, original_price: 0, currency: '', validity_days: 30, validity_unit: 'days', purchase_limit_per_user: 0, sort_order: 0, for_sale: true })
...
watch(() => props.show, (visible) => {
  if (!visible) return
  if (props.plan) {
    Object.assign(planForm, { name: props.plan.name, group_id: props.plan.group_id, description: props.plan.description, price: props.plan.price, original_price: props.plan.original_price || 0, currency: props.plan.currency || '', validity_days: props.plan.validity_days, validity_unit: props.plan.validity_unit || 'days', purchase_limit_per_user: props.plan.purchase_limit_per_user || 0, sort_order: props.plan.sort_order || 0, for_sale: props.plan.for_sale })
    planFeaturesText.value = (props.plan.features || []).join('\n')
  } else {
    Object.assign(planForm, { name: '', group_id: null, description: '', price: 0, original_price: 0, currency: '', validity_days: 30, validity_unit: 'days', purchase_limit_per_user: 0, sort_order: 0, for_sale: true })
...
function buildPlanPayload() {
  ...
  return {
    ...
    purchase_limit_per_user: planForm.purchase_limit_per_user || 0,
