...
describe('resolveDocumentTitle', () => {
  ...
  it('站点名为空时，回退默认站点名', () => {
    expect(resolveDocumentTitle('Dashboard', '')).toBe('Dashboard - SuperAI')
    expect(resolveDocumentTitle(undefined, '   ')).toBe('SuperAI')
  })
...
describe('resolveRouteDocumentTitle', () => {
  it('自定义页面菜单加载后，使用菜单名称作为标题', () => {
    ...
    expect(resolveRouteDocumentTitle(route, 'SuperAI')).toBe('Custom Page - SuperAI')
    expect(resolveRouteDocumentTitle(route, 'SuperAI', [
      {
        id: 'scheduler',
        label: '账号调度器',
        icon_svg: '',
