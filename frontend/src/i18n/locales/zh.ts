export default {
  ...
  admin: {
    ...
    upstreamMonitor: {
      ...
      sources: {
        title: '上游源管理',
        description: '把你接入的上游逐条录入，包括价格接口、鉴权方式、币种与说明，后续检测都从这里读取。',
        add: '新增上游源',
        empty: '还没有配置任何上游源。',
        ...
        noUrl: '尚未填写接口地址',
        noGroups: '未绑定分组',
        syncHint: '自动同步目前支持两类返回：1）JSON 接口；2）纯数字文本。若是复杂网页，请尽量提供一个可直接读取倍率的 API 地址。',
        pullGroups: '拉取上游分组',
        pullingGroups: '拉取中',
        pullGroupsHint: '先填写价格接口，再拉取分组。',
        applyPreset: '套用接口',
        applyPresetSuccess: '已根据上游类型生成默认接口配置。',
        applyPresetMissingBaseUrl: '请先填写站点地址，例如 https://pool.gptstore.club',
        pullGroupsMissingUrl: '请先填写价格接口地址',
        ...
        basic: {
          title: '关键配置',
          description: '日常只需要填写这些：上游名称、价格接口、参考倍率、是否自动同步，以及这个上游覆盖哪些本地分组。',
        },
        advanced: {
          title: '高级设置',
        ...
        fields: {
          ...
          autoSync: '自动同步',
          autoSyncHint: '开启后可根据价格接口自动回填参考倍率。',
          fetchMode: '抓取方式',
          baseUrl: '站点地址',
          baseUrlPlaceholder: 'https://pool.gptstore.club',
          baseUrlHint: '填写对方中转站首页域名，点击“套用接口”会自动生成价格接口。',
          pricingUrl: '价格接口地址',
          ...
          sourceReferenceFallback: '使用上游源参考倍率',
          bindMapping: '绑定映射',
          updateMapping: '更新映射',
          ...
          upstreamGroupEmptyHint: '填写价格接口并点击“拉取上游分组”后，这里才会出现可选择的上游分组。',
          pathFallback: '无 ID，按路径匹配',
          upstreamGroup: '上游中转分组名',
          upstreamGroupHint: '填写对方中转站里的分组名；不填时默认等于本地分组名。',
          mappingReferenceMultiplier: '该映射上游倍率',
          mappingReferenceMultiplierHint: '如果这个上游不同分组倍率不同，在这里填写；留空或 0 时使用上游源的参考倍率。',
          mappingReferenceMultiplierAutoHint: '已绑定上游分组，倍率会使用最新拉取到的上游分组倍率；需要手动填写时请切回手动上游分组。',
          status: '当前状态',
          notes: '备注',
        },
      },
